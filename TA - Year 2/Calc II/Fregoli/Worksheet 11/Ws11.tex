...
\usepackage{amsmath,amsfonts,fullpage,amsthm,graphicx,verbatim,tikz}
...
\noindent
\begin{align*}
u &= \sin (x)  &du= \hspace{1.25in} u &= \sec (x) &du = \hspace{1in}\\
u &= \cos (x)  &du= \hspace{1.25in} u &= \csc (x) &du = \hspace{1in}\\
u &= \tan (x)  &du= \hspace{1.25in} u &= \cot (x) &du = \hspace{1in}\\
\end{align*}
...
\begin{enumerate}
\item First, substitute $x=2\sec(\theta)$. Convert the integral completely into the variable $\theta$. (Hint: If $x=2\sec(\theta)$ then what does $dx$ equal?) 
...
\begin{tikzpicture}
	...
	\node  at (1.5,-1.8) {$\theta$}; 
	\node[left]  at (0,-1) {opposite}; 
	\node[below]  at (1,-2) {adjacent}; 
	\node[right] at (1,-1) {hypotenuse}; 
\end{tikzpicture}
