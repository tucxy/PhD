...
\newpage
\begin{thm}
If $R$ is a Noetherian Domain and $P\subset R$ is a prime ideal minimal over $I_{n}=\langle p_{1},\hdots, p_{n}\rangle$ where $p_{1},\ldots,p_{n}$ are primes, then $\h(P)\leq n.$
\end{thm}

\begin{proof}
Recall that by \textbf{Lemma~\ref{lem:minprime}}, there exists a minimal prime ideal $P$ over any ideal $I$ in $R$. Let $I=\langle p_{1},\hdots, p_{n}\rangle$ where $p_{1},\ldots,p_{n}$ are primes, and let $P$ be a minimal prime ideal over $I$. Now recall that the localization $R_{P}$ of $R$ at $P$ defined via
    $$R_{P}=\left\{\frac{r}{q}: r\in R, q\in R\setminus P\right\}\text{ is a local ring with maximal ideal }PR_{P}\text{ by \textbf{Homework 3}.}$$

If $n=1$, $I=\langle p_{1}\rangle$ and since $p_{1}$ is a prime element, $I$ is a prime ideal, and $P=\langle p_{1}\rangle$. By \textbf{Lemma~\ref{lem:NoP}}, $\mathrm{ht}(P)\leq 1$.

Suppose the statement holds for some $k\geq 1$. Let $I=\langle p_{1},\hdots,p_{k},p_{k+1}\rangle$ where $p_{1},\hdots,p_{k+1}$ are primes, and let $P$ be a prime ideal minimal over $I$..

Consider any chain $Q_{1}\subseteq Q_{2}\subseteq \cdots \subseteq Q_{k}=Q$ of prime ideals terminating at $Q$, and let $j\in [k]$ be maximal such that $Q_{j}\subset Q$. Then $Q_{1}\subset \cdots \subset Q_{j}\subset Q$, so $j\leq \mathrm{ht}(Q)\leq k$.

Since $Q\subseteq P$, we may extend this chain by adding $P$ at the end:
\[
Q_{1}\subset \cdots \subset Q_{j}\subset Q\subseteq P.
\]

Passing to the quotient $R/Q$, any strict containment between $Q$ and $P$
corresponds to a strict containment between $0$ and $P/Q$. Since $R/Q$ is a
Noetherian domain and $P/Q$ is a prime ideal minimal over
$\langle\class[Q]{p_{k+1}}\rangle$, Lemma~\ref{lem:NoP} gives
$\mathrm{ht}(P/Q)\le 1$. Hence there can be at most one strict containment
between $Q$ and $P$.

Therefore any chain of prime ideals terminating at $P$ has length at most
$j+1\le k+1$, so $\mathrm{ht}(P)\le k+1$.
\end{proof}
...
\textbf{(a)} \begin{thm}
If $R$ is a Noetherian UFD and $P=\langle p_{1},\hdots,p_{n}\rangle$ is a prime ideal generated by $n$ prime elements, then $\mathrm{ht}(P)\leq n$.
\end{thm}

\begin{proof}
We prove the stronger statement that if $I=\langle p_{1},\hdots,p_{n}\rangle$ is an ideal generated by $n$ prime elements and $P$ is a prime ideal minimal over $I$, then $\mathrm{ht}(P)\leq n$. The result follows immediately since if $P=\langle p_{1},\hdots,p_{n}\rangle$ is itself prime, then $P$ is minimal over itself.

If $n=1$, let $P$ be prime minimal over $\langle p_{1}\rangle$. Since $p_{1}$ is a prime element, $\langle p_{1}\rangle$ is a prime ideal, so $P=\langle p_{1}\rangle$. By Lemma~\ref{lem:NoP}, $\mathrm{ht}(P)\leq 1$.

Suppose the statement holds for $n=k$. Let $I=\langle p_{1},\hdots,p_{k},p_{k+1}\rangle$ and let $P$ be a prime ideal minimal over $I$. Set $J=\langle p_{1},\hdots,p_{k}\rangle$. Since $R$ is Noetherian, there exists a prime ideal $Q$ minimal over $J$. By the induction hypothesis, $\mathrm{ht}(Q)\leq k$.

Let $P_{0}\subsetneq P_{1}\subsetneq\cdots\subsetneq P_{m}=P$ be a chain of prime ideals. Let $i$ be the largest index such that $P_{i}\subseteq Q$. Then $P_{0}\subsetneq\cdots\subsetneq P_{i}\subseteq Q$, so $i\leq\mathrm{ht}(Q)\leq k$.

Since $P$ is minimal over $I$, we have $I\subseteq P_{i+1}$. Thus $p_{1},\hdots,p_{k}\in P_{i+1}$, so $Q\subseteq P_{i+1}$. Passing to the quotient $R/Q$, the ideals $P_{i+1}/Q\subsetneq\cdots\subsetneq P_{m}/Q=P/Q$ form a chain of prime ideals. Since $R/Q$ is a Noetherian domain and $P/Q$ is a prime ideal minimal over $\langle\class[Q]{p_{k+1}}\rangle$, Lemma~\ref{lem:NoP} gives $\mathrm{ht}(P/Q)\leq 1$. Hence $m-i\leq 1$.

Therefore $m=i+(m-i)\leq k+1$, so $\mathrm{ht}(P)\leq k+1$. This completes the induction.
\end{proof}
