...
\begin{prob}
Assume $\KK$ is a field of characteristic $0$. Let $G$ be the subgroup of $\operatorname{Aut}_{\KK}\!\big(\KK(x)\big)$ generated by the $\KK$-automorphism induced by $x\mapsto x+1$. Prove that $G$ is an infinite cyclic group. What is the fixed field $\EE$ of $G$? What is $[\KK(X):\EE]$?
\end{prob}
\begin{proof}
...
Alright now let's do the problem. I just wanted to prove this all myself instead of looking over the notes. For now on I will refer to any finite field $\KK$ as $\FF_{p^{n}}$.
\newpage
\setcounter{thm}{60} % next prob is 61
\newpage
\begin{prob}
Let $\KK$ be a finite field of characteristic $p>0$.
\begin{enumerate}[label=(\alph*)]
\item Prove that for every $n>0$ there exists an irreducible polynomial $f\in \KK[X]$ of degree $n$.
\item Prove that for every irreducible polynomial $P\in \KK[X]$ there exists $n\ge 0$ such that $P$ divides $X^{p^{\,n}}-X$.
\end{enumerate}
\end{prob}
\begin{proof}
...
%\begin{align*}
%  &\phi_{b,-b}^{2}\coloneq\phi_{b,-b}(\phi_{b,-b}(b))=\phi_{b,-b}(-b)=b\implies \phi_{b,-b}^{2}=\id.\\
%  &\phi_{a,ba}^{2}\coloneq \phi_{a,ba}(ba)=\phi_{ba}(b)\phi_{ba}(a)=b(ba)=-a\implies \phi_{a,ba}^{3}\coloneq \phi_{a,ba}(-a)=-ba\\
%  \implies &\phi_{a,ba}^{4}\coloneq \phi_{a,ba}(-ba)=-b(ba)=a\implies \phi_{a,ba}^{4}=\id\\
%  \implies &\phi_{a,-a}=\phi_{a,-ba}^{2},\phi_{a,-ba}=\phi_{a,ba}^{3},|\phi_{b,-b}|=2,\text{ and }|\phi_{a,ba}|=4.
%\end{align*}
%Furthermore, since $\langle \phi_{b,-b}\rangle\cap \langle \phi_{a,ba}\rangle=\{\id\}$, we must have that $|\langle \phi_{b,-b}\rangle\langle \phi_{a,ba}\rangle|=\frac{(2)(4)}{1}=8=|G|$ and so $\Gal{\mathbb{Q}(a,b)}{\QQ}=\langle\phi_{b,-b},\phi_{a,ba}\rangle$. Well, $(\phi_{b,-b}\circ \phi_{a,ba})(a)=\phi_{b,-b}(ab)=-ab\neq ba=\phi_{a,ba}(a)=(\phi_{a,ba}\circ \phi_{b,-b})(a)$. Therefore, $\phi_{b,-b}\circ \phi_{a,ba}\neq \phi_{a,ba}\circ \phi_{b,-b}$, and $\Gal{\mathbb{Q}(a,b)}{\QQ}$ is a non-abelian group of order $8$ with an element $\phi_{b,-b}$ of order $2$. So then since it could only be isomorphic to $Q_{8}$ or $D_{4}$ purely by order, and $Q_{8}$ has no elements of order $2$, we must have that $\Gal{\mathbb{Q}(a,b)}{\QQ}\cong D_{4}$.
