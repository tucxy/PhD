...
\begin{proof}
  Let $p(x)=x^{4}+x^{3}+1\in \FF_{2}$. $p(0)=p(1)=1\neq 0$, so $p(x)$ has no zeros in $\FF_{2}$ and therefore no linear factors over $\FF_{2}$, which means it can't factor into a linear and cubic. Suppose $p(x)$ is reducible. Then must split into two irreducible quadratics over $\FF_{2}$. These quadratics must have a non-zero constant and leading coefficient, which must then be $1\in \FF_{2}$, otherwise $x$ would divide them along with $p(x)$ or they wouldn't be quadratic at all. So then a factor $p(x)$ could only be $x^{2}+x+1$ or $x^{2}+1$. However, notice that $x^{2}+1$ has $1$ as a zero, so $x-1$ divides it, and would have to then divide $p(x)$. Therefore, we must have that $p(x)$ has only one possible (repeated) factor: $x^{2}+x+1$. Well, since $\Char{\FF_{2}}=2$, we have Freshman's dream for multiples of $2$ and so $(x^{2}+x+1)^{2}=((x^{2})+(x+1))^{2}=x^{4}+x^{2}+2x+1=x^{4}+x^{2}+1\neq x^{4}+x^{3}+1=p(x)$, a contradiction. So $p(x)$ must be irreducible over $\FF_{2}.$

  Since $p(x)$ is monic and irredicuble, $\langle p(x)\rangle$ is a maximal ideal, and so $\EE=\FF_{2}[x]/\langle x^{4}+x^{3}+1\rangle$ is a field extension that is also a $4$-dimensional $\FF_{2}$-vector space. Therefore $|\FF_{2}[x]/\langle x^{4}+x^{3}+1\rangle|2^{4}=16$. Now we will just use the shorthand $x\coloneq [x]$ since we work in $\EE$ for the remainder of this proof. 

  $\EE=\Span\{1,x,x^{2},x^{3}\}$. Now, since $x^{4}+x^{3}+1=0$ in this field, $x^{4}=-x^{3}-1=x^{3}+1$, and obviously $x$ is a root of $p(x)=x^{4}+x^{3}+1$ over $\EE$. Then, recall that since $\EE\cong \FF_{2^{4}}$, a finite field with characteristic $2$, we must have that the Frobenius endomorphism $\varphi_{2}(x)=x^{2}$ is an automorphism in $\mathrm{Aut}_{\FF_{2}}(\FF_{2^{4}})$. So then it permutes roots of any $f(x)\in \FF_{2}$ in any extension of $\FF_{2}$ to each other. Therefore, the orbit $\langle x\rangle\varphi_{2}=\{x\}\cup \{x^{2}\}\cup \{x^{4}=x^{3}+1\} \cup \{x^{8}=(x^{4})^{2}=(x^{3}+1)^{2}=x^{6}+1=(x^{3}+x^{2}+x+1)+1=x^{3}+x^{2}+x\}\cup \cdots = \{x,x^{2},x^{3}+1,x^{3}+x^{2}+x\}$ must be all roots of $p(x)$ over $\EE$, since they are four distinct elements and $p(x)$ has at most four distinct roots.

\end{proof}

Heres the computation used to find $x^{6}$. $x^{4}=x^{3}+1\implies x^{5}=x(x^{4})=x(x^{3}+1)=x^{4}+x=x^{3}+x+1\implies x^{6}=x(x^{5})=x(x^{3}+x+1)=x^{4}+x^{2}+x=(x^{3}+1)+x^{2}+x=x^{3}+x^{2}+x+1$
\newpage
\begin{prob}
